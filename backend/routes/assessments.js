...
async function requireMarkEntryAccess(req,sid){
  ...
  const q=await db.query(`SELECT 1 FROM user_school_roles WHERE user_id=$1 AND school_id=$2 AND role=$3 AND mark_entry_enabled`,[req.user.user_id,sid,req.user.role]);
    ...
    const q=await db.query(`
      WITH ranked AS (
        SELECT a.*,
               ...
               ROW_NUMBER() OVER (
                 PARTITION BY
                   CASE WHEN a.assessment_type IN ('OPENER','MID_TERM','END_TERM') THEN a.assessment_type ELSE a.id::text END,
        ...
        WHERE a.school_id=$1 AND a.academic_year_id=$2 AND a.term_id=$3
          AND (
            (t.term_no IN (1,2) AND a.assessment_type IN ('OPENER','MID_TERM','END_TERM'))
            OR (t.term_no=3 AND a.assessment_type IN ('OPENER','END_TERM'))
            OR a.assessment_type NOT IN ('OPENER','MID_TERM','END_TERM')
          )
      )
      SELECT * FROM ranked
      WHERE standard_rank=1
      ORDER BY CASE
        WHEN assessment_type='OPENER' THEN 1
        WHEN assessment_type='MID_TERM' THEN 2
        WHEN assessment_type='END_TERM' THEN 3
        ELSE 5
      END, assessment_order, created_at`,
      [req.params.schoolId, req.params.yearId, req.params.termId]
    );
    res.json(q.rows);  }catch(e){
...
router.post('/:schoolId/:yearId/:termId/class-bulk',schoolBoundary,roles('SCHOOL_ADMIN','ADMIN'),async(req,res)=>{
  ...
  try{
    ...
    const allowedMain=termRow.term_no===3?new Set(['OPENER','END_TERM']):new Set(['OPENER','MID_TERM','END_TERM']);
    const filteredTypes=types.filter(x=>x.startsWith('CAT')||allowedMain.has(x));
    if(!filteredTypes.length)return res.status(400).json({error:`No valid assessments selected for Term ${termRow.term_no}.`});
...
router.post('/:schoolId/marks/save',schoolBoundary,roles('SCHOOL_ADMIN','ADMIN','HEADTEACHER','DEPUTY_HEADTEACHER','SENIOR_TEACHER','CLASS_TEACHER','TEACHER'),async(req,res)=>{
  ...
  try{
    ...
    const assessment=(await client.query(`
      SELECT id,max_mark,academic_year_id,term_id,deadline_at,status,class_id,subject_id
    ...
    for(const row of rows){
      const learner=(await client.query(
        `SELECT id FROM learners WHERE id=$1 AND school_id=$2 AND class_id=$3`,
        [row.learner_id,sid,assignment.class_id]
      )).rows[0];
      ...
      await client.query(`
        ...
        [sid,row.learner_id,assignment.subject_id,assignment.class_id,assignment.academic_year_id,
         assessment.term_id,b.assessment_id,mark,req.user.user_id,null]
      );
      saved++;
    }
...
router.post('/:schoolId/:yearId/:termId',schoolBoundary,roles('SCHOOL_ADMIN','ADMIN'),async(req,res)=>{
  ...
  try{
    ...
    const standardForTerm=term.term_no===3?new Set(['OPENER','END_TERM']):new Set(['OPENER','MID_TERM','END_TERM']);
...
router.patch('/:schoolId/:yearId/:termId/:assessmentId',schoolBoundary,roles('SCHOOL_ADMIN','ADMIN'),async(req,res)=>{
  try{
    ...
    const q=await db.query(`
      ...
      WHERE id=$3 AND school_id=$4 AND academic_year_id=$5 AND term_id=$6
      RETURNING *
    `,[deadline,status,assessmentId,sid,yearId,termId]);
    res.json(q.rows[0]);
  }catch(e){
...
router.get('/:schoolId/assignment/:assignmentId/roster',schoolBoundary,roles('SCHOOL_ADMIN','ADMIN','HEADTEACHER','DEPUTY_HEADTEACHER','SENIOR_TEACHER','CLASS_TEACHER','TEACHER'),async(req,res)=>{
  ...
  try{
    ...
    const assessments=(await db.query(`
      SELECT a.id,a.name,a.assessment_type,a.max_mark,a.weight,a.status,a.deadline_at,a.class_id,a.subject_id,a.academic_year_id,a.term_id,a.assessment_order
      FROM assessments a
      WHERE a.school_id=$1 AND a.academic_year_id=$2
        ...
        ${termSql}
      ORDER BY CASE WHEN a.assessment_type='OPENER' THEN 1 WHEN a.assessment_type='CAT' THEN 2 WHEN a.assessment_type='MID_TERM' THEN 3 WHEN a.assessment_type='END_TERM' THEN 4 ELSE 5 END,a.assessment_order,a.created_at`,assessmentParams)).rows;
...
router.get('/:schoolId/assignment/:assignmentId/marks-pdf',schoolBoundary,roles('SCHOOL_ADMIN','ADMIN','HEADTEACHER','DEPUTY_HEADTEACHER','SENIOR_TEACHER','CLASS_TEACHER','TEACHER'),async(req,res)=>{
  try{
    ...
    res.setHeader('Content-Disposition',`attachment; filename="mark-sheet-${String(assignment.class_name||'class').replace(/[^a-z0-9]+/gi,'-').toLowerCase()}-${String(assignment.subject_code||'subject').replace(/[^a-z0-9]+/gi,'-').toLowerCase()}-${String(a.name||'assessment').replace(/[^a-z0-9]+/gi,'-').toLowerCase()}.pdf"`);
    doc.pipe(res);
