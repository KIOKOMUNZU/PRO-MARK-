...
async function getContext(schoolId, learnerId, yearId, termId) {
  ...
  if(!meta.next_term_opening_date){
    const nxt=(await db.query(`SELECT t.id term_id,t.term_no next_term_no,t.name next_term_name,t.opening_date next_term_opening_date,t.closing_date next_term_closing_date
      ...
      ORDER BY ay.start_date ASC NULLS LAST,ay.year_label ASC LIMIT 1`,[schoolId,meta.closing_date||null])).rows[0];
    if(nxt){meta.next_term_id=nxt.term_id;meta.next_term_no=nxt.next_term_no;meta.next_term_name=nxt.next_term_name;meta.next_term_opening_date=nxt.next_term_opening_date;meta.next_term_closing_date=nxt.next_term_closing_date;meta.next_term_reopening_date=nxt.next_term_reopening_date;}
  }
...
async function buildTermReportCore(schoolId, learnerId, yearId, termId) {
  ...
  // These two queries are independent. Run them together so report-card opens do
  // not pay the database round-trip latency twice.
  const [rankResult,priorResult]=await Promise.all([
    db.query(`
    ...
    FROM per_learner_subject`,[schoolId,meta.class_id,yearId,termId]),
    db.query(`
    ...
    WHERE m.school_id=$1 AND m.learner_id=$4 AND m.mark IS NOT NULL
      AND a.assessment_type IN (${reportAssessmentSql})
    GROUP BY ay.year_label,m.subject_id`,[schoolId,yearId,meta.term_no,learnerId])
  ]);
  ...
  const stamp=(await db.query(`SELECT file_url FROM school_document_assets WHERE school_id=$1 AND asset_type='STAMP' AND is_active=true AND (effective_to IS NULL OR effective_to>=CURRENT_DATE) ORDER BY effective_from DESC NULLS LAST,created_at DESC LIMIT 1`,[schoolId])).rows[0];
...
async function buildTermReport(schoolId,learnerId,yearId,termId){
  return cached(reportCacheKey(schoolId,learnerId,yearId,termId),15000,()=>buildTermReportCore(schoolId,learnerId,yearId,termId));
}
