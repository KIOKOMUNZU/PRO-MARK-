...
CREATE TABLE IF NOT EXISTS assessment_configurations(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),school_id uuid REFERENCES schools(id) ON DELETE CASCADE,section_id uuid REFERENCES school_sections(id),level_id uuid REFERENCES school_levels(id),assessment_id uuid REFERENCES assessments(id),grading_system_id uuid REFERENCES grading_systems(id),include_in_term_average boolean DEFAULT true,include_in_merit boolean DEFAULT true,created_at timestamptz DEFAULT now());
